...
carts_router.get('/',async(req,res,next)=> {
    try {
        let all = await Cart.find().select('user_id product_id -_id')
            .populate('user_id','name -_id')
            .populate('product_id','title -_id')
...
carts_router.get('/users/:uid',async(req,res,next)=> {
    try {
        ...
        let all = await Cart.find({ user_id:uid }).select('user_id product_id -_id')
        return res.status(200).json({
            success:true,
            response:all
        })
...
carts_router.get('/bills/:uid', async(req,res,next)=> {
    try {
        let all = await Cart.aggregate([
            ...
            { $project: { product_id: 0, quantity: 0, price: 0, __v: 0, active: false } },
